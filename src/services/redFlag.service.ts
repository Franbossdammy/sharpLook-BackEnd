...
class RedFlagService {
   ...
   */
  public async checkProximityOnLocationUpdate(
    userId: string,
    ...
    try {
      ...
      const nearbyUsers = await User.find(oppositeQuery).select('_id location').limit(20).lean();

      for (const nearby of nearbyUsers) {
        if (!nearby.location?.coordinates) continue;
        const nearbyCoords = nearby.location.coordinates as [number, number];
      ...
      logger.error('Proximity check on location update failed:', err);
    }
  }
   ...
   */
  public async runProximitySweep(): Promise<void> {
    try {
      const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000);

      // Get online vendors with a recent location ping
      const onlineVendors = await User.find({
        isVendor: true,
        isOnline: true,
      ...
      for (const vendor of onlineVendors) {
        ...
        const vendorCoords = vendor.location.coordinates as [number, number];

        // Find online clients within 500m of this vendor
        const nearbyClients = await User.find({
          isVendor: false,
          isOnline: true,
          lastSeen: { $gte: tenMinutesAgo },
          'location.coordinates': {
        ...
        for (const client of nearbyClients) {
          if (!client.location?.coordinates) continue;
          const clientCoords = client.location.coordinates as [number, number];
          await this.detectLocationProximity(
            vendor._id.toString(),
            client._id.toString(),
            vendorCoords,
            clientCoords,
      ...
      if (pairsChecked > 0) {
        logger.info(`Proximity sweep checked ${pairsChecked} vendor–client pairs`);
      }
    } catch (err) {
      logger.error('Proximity sweep failed:', err);
    }
  }
   ...
   * Run from cron daily.
   */
  public async runDropoutDetection(
    silentDays: number = 45,
    minBookings: number = 2
  ): Promise<void> {
    try {
      ...
      const pairs = await Booking.aggregate([
      ...
      logger.info(`Dropout detection: found ${pairs.length} silent vendor–client pairs`);
      ...
      for (const pair of pairs) {
        ...
        const existingFlag = await RedFlag.findOne({
          type: RedFlagType.REPEAT_CLIENT_DROPOUT,
          flaggedUser: vendorId,
          relatedUser: clientId,
        ...
        const vendor = await User.findById(vendorId).select('firstName lastName vendorProfile status isVendor').lean();
        const client = await User.findById(clientId).select('firstName lastName status').lean();

        if (!vendor || !client) continue;
        if ((vendor as any).status === 'suspended' || (vendor as any).status === 'inactive') continue;
        if (!vendor.isVendor) continue;

        const vendorName = (vendor as any).vendorProfile?.businessName || `${(vendor as any).firstName} ${(vendor as any).lastName}`;
        const clientName = `${(client as any).firstName} ${(client as any).lastName}`;
        ...
        logger.warn(`🚩 Dropout flag: ${vendorName} + ${clientName} — ${pair.totalBookings} bookings, silent ${daysSilent} days`);
      }
    } catch (err) {
      logger.error('Dropout detection failed:', err);
    }
  }
}
