...
function getAction(method: string, path: string): string {
  ...
  if (lowerPath.includes('/cancel')) return 'CANCEL';
  if (lowerPath.includes('/accept')) return 'ACCEPT';
  if (lowerPath.includes('/start')) return 'START';
  if (lowerPath.includes('/complete')) return 'COMPLETE';
  if (lowerPath.includes('/resolve')) return 'RESOLVE';
  if (lowerPath.includes('/assign')) return 'ASSIGN';
  if (lowerPath.includes('/escalate')) return 'ESCALATE';
  if (lowerPath.includes('/hide')) return 'HIDE';
  if (lowerPath.includes('/unhide')) return 'UNHIDE';
  if (lowerPath.includes('/toggle')) return 'TOGGLE';
  if (lowerPath.includes('/verify')) return 'VERIFY';
...
function buildDetails(action: string, resource: string, req: AuthRequest): string {
